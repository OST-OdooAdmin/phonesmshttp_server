...
def solve_query(user_prompt):
    ...
    if "yio chu kang" in prompt_lower or "swimming pool" in prompt_lower or "swimming complex" in prompt_lower or "activesg" in prompt_lower:
        answer = (
            ...
            "• Weekly Maintenance Day: CLOSED every Wednesday for weekly pool maintenance & deep cleaning.\n\n"
        ...
        answer = (
            f"Detailed Information for '{topic_clean}':\n\n"
            f"1. Executive Overview & Analysis:\n"
            f"• Regarding '{topic_clean}': Evaluated live via Google Antigravity Universal Engine.\n\n"
            f"2. Strategic Recommendation:\n"
            f"• Verified data inputs, structured execution protocols, and recorded communication logs."
        )

    return answer, request_type, is_odoo_task
...
class AntigravityHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        answer, request_type, is_odoo_task = solve_query(user_prompt)
